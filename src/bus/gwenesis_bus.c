...
void load_cartridge(unsigned char *buffer, size_t size)
{
    ...
    //
    // NOTE: this used to realloc()+memcpy() the whole ROM into a fresh
    // heap buffer here (the #else branch below, historically for
    // desktop/non-retro-go builds with plenty of RAM). On this EADK port
    // there's only a few hundred KB of RAM total for the whole app, but
    // ROMs can be hundreds of KB to several MB - that realloc() was
    // silently failing (returning NULL) and the following memcpy() into
    // a NULL pointer was crashing/rebooting the calculator. buffer here
    // already points at eadk_external_data (flash-backed, valid for the
    // app's whole lifetime), so we just alias ROM_DATA to it directly -
    // same as the RETRO_GO branch does, without pulling in the RETRO_GO
    // macro (which gates unrelated retro-go-framework code elsewhere in
    // this file that we don't want).
    //
    // Caveat: the SMD de-interleave block just below writes into
    // ROM_DATA in place if the header matches an interleaved .smd ROM.
    // Since ROM_DATA now points at flash instead of a heap copy, that
    // would fail for genuinely interleaved ROMs. Not an issue for
    // standard .gen/.bin format ROMs (no header match, block skipped),
    // but worth fixing properly (e.g. de-interleaving into a small
    // scratch buffer) before supporting .smd ROMs.
    ROM_DATA = buffer;
    ...
    // ROM_SWAP is meant only for byte-swapped ROM dumps (an alternate dump
    // format some old flash carts used) - it is NOT needed for a standard
    // .gen ROM like sonic.gen. It was found to be unexpectedly defined
    // (not from this file, not from the Makefile's CFLAGS - likely a
    // leftover retro-go platform header pulled in via one of the include
    // paths) and its full in-place 512KB read-modify-write loop over
    // ROM_DATA was crashing the calculator (confirmed via bisection: a
    // single-byte write to ROM_DATA succeeds fine, so it's specific to
    // the full-size loop, not ROM_DATA's writability in general). Since
    // this feature isn't needed for supported ROMs, force it off here
    // rather than debug the loop itself.
    #undef ROM_SWAP
